...
async function scanSessionLogs(
  ...
  try {
    sessionEntries = await fs.promises.readdir(sessionsDir, {
      withFileTypes: true,
    });
  } catch {
    return [];
...
class CostComponent {
  ...
  private async load() {
    const days = this.tabs[this.currentTab]?.days ?? null;
  ...
  render(width: number): string[] {
    ...
    if (this.loading) {
      ...
      for (const pc of this.costs) {
        ...
        lines.push(
          box(
            `${cursorMark}${bold(pc.displayName.toLowerCase())} ${color(costStr)} ${dim(`(${pc.totalRequests} requests)`)}`,
          ),
        );
